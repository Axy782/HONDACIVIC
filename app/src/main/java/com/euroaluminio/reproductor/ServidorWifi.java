...
public class ServidorWifi {
    ...
    private String paginaHtml() {
        return "<!DOCTYPE html><html lang='es'><head><meta charset='utf-8'>"
            ...
            + "<p>Canciones (MP3) &rarr; carpeta Descarga. Videos (MP4) &rarr; carpeta Videos. APK y otros archivos &rarr; carpeta Archivos. Todo en la memoria USB.</p>"
            + "<input id='f' type='file' accept='*/*' multiple style='display:none'>"
            + "<button class='btn' onclick=\"document.getElementById('f').click()\">+ Elegir archivos (musica, video, APK...)</button>"
            ...
            + "f.onchange=function(){var fs=f.files;if(!fs.length)return;lista.style.display='block';"
            + "for(var k=0;k<fs.length;k++){var file=fs[k];var row=document.createElement('div');row.className='fila prog';row.textContent='En espera: '+file.name;lista.appendChild(row);cola.push({file:file,row:row});}"
            + "f.value='';procesar();};"
            + "function procesar(){if(subiendo||!cola.length)return;subiendo=true;var it=cola.shift();var file=it.file,row=it.row;"
            + "row.className='fila prog';row.textContent='Enviando: '+file.name+' ...';"
            ...
            + "xhr.onload=function(){if(xhr.status==200){row.className='fila ok';row.textContent=(xhr.responseText||'OK')+' - '+file.name;}else{row.className='fila err';row.textContent=(xhr.responseText||'Error')+' : '+file.name;}subiendo=false;procesar();};"
            + "xhr.onerror=function(){row.className='fila err';row.textContent='Error de conexion: '+file.name;subiendo=false;procesar();};"
            + "xhr.send(file);}"
            + "</script></body></html>";
    }
}
